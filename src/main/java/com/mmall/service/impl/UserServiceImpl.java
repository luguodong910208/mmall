...
public class UserServiceImpl implements IUserService {
	...
	public ServerResponse<String> forgetResetPassword(String username, String password, String forgetToken){
		...
		if(StringUtils.isBlank(token)){
			return ServerResponse.createByErrorMessage("token失效或者过期");
		}
	...
	public ServerResponse<String> resetPassword(String passwordNew, String passwordOld, User user){
		//放置横向越权，要校验一个这个用户的旧密码，一定要制定是这个用户，因为我们会查询一个count(1)，如果不指定id，那么结果就是true即count>0
		...
		if(updateCount > 0){
			return ServerResponse.createBySuccessMessage("密码 更新成功");
		}
	...
	public ServerResponse<User> updateInformation(User user){
		//username是不能进行更改的
		//email需要进行一个校验，校验新的email是不是已经存在，并且存在的email如果相同的话，不能是我们当前的用户
